...
const applyRules = (cartItems, rules) => {
  if (cartItems.length === 0) return cartItems;

  const bXgYRules = rules.filter(
    (rule) =>
  ...
  const buyXGetYAdjustments = buy_x_get_y_adjustments(cartItems, bXgYRules);
  return buyXGetYAdjustments;
};
